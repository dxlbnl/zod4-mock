...
- The code panel shows bare schemas — nothing that explains *how* IDs get wired (the two-phase `generate` → `map FK` pattern is invisible)
...
Show the selected entity *fully resolved* — all FK UUIDs replaced with the actual nested entity object. Built with a `resolveEntity(entity, world)` utility:
