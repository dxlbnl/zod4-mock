...
type ProductData = { productId: string; sku: string; name: string; unitPriceCents: number }
...
const DocumentSubject = defineSubjectType('document', z.object({
  documentId: z.string().uuid(),
}))

const SentenceSubject = defineSubjectType('sentence', z.object({
...
function createDocumentCorpusWorld(seed = 42) {
  return createWorld({ seed })
    ...
    .withSchema(DocumentSchema, DocumentSubject, {
      ...
      language: (_, ctx) => ctx.registry.pick<{ language: 'nl' | 'en' | 'de' | 'fr' }>('author').language,
    })
...
**Key takeaways:**
- Subject schemas can be as minimal as `z.object({ documentId: z.string().uuid() })`. The subject only needs to carry the fields you reference in matchers.
...
function createMediaLibraryWorld(seed = 42) {
  return createWorld({
    ...
    .withSchema(RawDataSchema, TextFileSubject,  { id: (s) => s.fileId, type: () => 'text'  as const, sizeBytes: (s) => s.sizeBytes })
    .withSchema(RawDataSchema, AudioFileSubject, { id: (s) => s.fileId, type: () => 'audio' as const, sizeBytes: (s) => s.sizeBytes })
    .withSchema(RawDataSchema, BankFileSubject,  { id: (s) => s.fileId, type: () => 'bank'  as const, sizeBytes: (s) => s.sizeBytes })
...
**Key takeaways:**
- Calling `.withSchema(RawDataSchema, TextFileSubject, ...)` and `.withSchema(RawDataSchema, AudioFileSubject, ...)` means `generate(z.array(RawDataSchema))` produces one record per subject across *all* bound types.
...
const address = world.generate(z.object({
  street:     z.string(),
  city:       z.string(),
  postalCode: z.string(),
  country:    z.string(),
