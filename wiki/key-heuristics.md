...
| `name` *(not containing `file`)* | string schema | Full name (first + last) |
...
world.withGenerators({
  ...
  invoiceId: (_schema, ctx) =>
    `INV-${generators.uuid(ctx.prng).slice(0, 8).toUpperCase()}`,

  excerpt: (_schema, ctx) =>
    generators.loremText(ctx.prng, 15),
})
```
