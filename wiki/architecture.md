...
- All `generateArray` mode arms (derived, primary, ad-hoc) **MUST** apply the same trailing pass in the same order: cap to `callerMax ?? defMax`, apply per-index `options.overrides` (deepMerge per record), then apply `options.transform`. New behaviour added to one arm **MUST** be added to all three. When an **array** `options.overrides` is present, the element count is `override.length` (not the cap) — the "no resize" reading is superseded by D36. (→ D14)
...
- Sibling-read provenance for `TraceField.dependsOn` **MUST** be captured by a read-tracking `Proxy` over `ctx.current` installed **only** under the `createWorld({ trace: true })` gate (its `get`-trap records each sibling key a field's matcher reads); the off-path (`trace: false`, default) installs no Proxy, passes the bare record unchanged, and stays allocation- and PRNG-neutral (D4/D10). Reads **MUST NOT** be re-derived after the fact. (→ D33)
- Field-level `options.overrides` **MUST** be applied at a single per-field override-application site that dispatches on the override's shape (`undefined` → generate; primitive → replace without generating; plain object → `deepMerge` onto the generated value; array → generate then per-index merge, the override array length governing the element count — D36); pipeline rungs **MUST** stay override-agnostic, and there **MUST** be no post-record whole-record override merge. (→ D34)
- Generation under `store: false` **MUST** be value-neutral with respect to the store-on path: store-off registered-array elements **MUST** seed from explicit per-element indices `existingCount..existingCount+N-1` (identical to the store-on path's records at the same positions), so the i-th element's values do not depend on the `store` toggle. (→ D35)
- When `options.overrides` supplies an **array** for an array-typed target, the result **MUST** have exactly `override.length` elements (generated bases per-index-merged with their override slot: object → merge, primitive → replace, empty slot → fully generated); the override length **MUST** win even over an explicit `.length(N)`; schema bounds (`.length()`/`.min()`/`.max()`) and `defaultArrayLength` govern **only** the no-override case; the rule applies uniformly to nested-field, primary, derived, and ad-hoc paths, with the sole D8 carveout that a pre-populated primary array yields `max(existingCount, override.length)` elements (pre-existing stored records untouched). (→ D36)
