...
- Field-level `options.overrides` **MUST** be applied at a single per-field override-application site that dispatches on the override's shape (`undefined` → generate; primitive → replace without generating; plain object → `deepMerge` onto the generated value; array → generate then per-index merge, the override never resizing — schema length governs, B53-R2/D14); pipeline rungs **MUST** stay override-agnostic, and there **MUST** be no post-record whole-record override merge. (→ D34)
