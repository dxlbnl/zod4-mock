...
- All `generateArray` mode arms (derived, primary, ad-hoc) **MUST** apply the same trailing pass in the same order: cap to `callerMax ?? defMax`, apply per-index `options.overrides` (or throw per B38 on primary-registered inner schemas), then apply `options.transform`. New behaviour added to one arm **MUST** be added to all three. (→ D14)
