...
- **`flags: [review]`** — substantial scope (multi-package, type-shape break, data-shipping change). Spec page is needed before test-writer / implementer run; manager pauses at checkpoint #2 even though the *direction* is pre-decided, because the *acceptance shape* (file paths, exact LocaleData rename, changeset wording, dist size assertion approach) benefits from sign-off.
