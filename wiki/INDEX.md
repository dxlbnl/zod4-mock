...
| [research/overview.md](research/overview.md) | Generator-overhaul research — localization, Markov chains, PRNG, batching. Most pillars implemented; see `research/tracking.md`. |
| [research/tracking.md](research/tracking.md) | Live implementation-status checklist for the data-generation overhaul pillars (correctness, speed, data & realism). |
| [research/text-generation/markov-alternatives.md](research/text-generation/markov-alternatives.md) | B45 — alternatives to char-level Markov for name/word generation: measured baseline (531 KB EN words, 2.34 MB names), six-option comparison table (real-lists+Feistel, succinct storage, syllable n-grams, quantize/prune, neural-dismissed, real-lists+PCFG for words), determinism analysis of the Feistel/FPE unique-enumeration fit, recommendation + spike; notes how the direction subsumes B42 and feeds B2. |
| [research/reports/codebase-complexity.md](research/reports/codebase-complexity.md) | B22 — deep complexity survey of `src/`: per-function, module-shape, structural, and architectural hot spots, with refactor candidates. |
| [research/engine/generation-counter-d4-audit.md](research/engine/generation-counter-d4-audit.md) | B27 — audit of `WorldImpl.generationCounter`-derived PRNG fork keys vs D4's intent; confirms the call-order dependence is real on ad-hoc + array + outer-optional paths, recommends rename + documented rule (Option (a)) over identity-based fork keys (Option (b)). |
| [specs/B43-primary-array-min-max-ignored.md](specs/B43-primary-array-min-max-ignored.md) | B43 — BUG: `world.generate(primaryArraySchema.min(N).max(M))` silently ignores `.min/.max/.length` on a primary-registered inner schema; fix throws loudly (direction B) and redirects callers to `world.populate(schema, N)` for writing or `world.generate(schema.array()).slice(0, N)` for reading. Symmetric with B38 (same primary-mode arm of `generateArray`); D8-preserving by construction; ad-hoc + derived branches unaffected; minor bump. Closes #25. |
| [specs/B44-primary-array-store-false-hangs.md](specs/B44-primary-array-store-false-hangs.md) | B44 — BUG: `world.generate(primaryArraySchema, { store: false })` hangs forever because the primary-mode arm's `while (registry.count(innerSchema) < target)` loop never advances when B10-R4 transitive suppression skips the registry write. Fix decouples the loop's progress counter from the registry under `!effectiveStore`: generate `target` records directly via `Array.from(...)` and return that array (B20's "Fix B" local-capture precedent, primary-array analogue). Store-on path byte-identical (B44-R5); B10-R2/R4 invariants restored; ad-hoc + derived branches unaffected; orthogonal to B38 (overrides throw) and B43 (`.min/.max` throw) — both throws still fire first on composed calls (B44-R8); patch bump (pure bug fix, no contract change for any non-hanging caller). Closes #26. |
