...
switch (mode.kind) {
  case "derived": { /* iterate source pairs */ }
  case "primary": { /* registered-array path */ }
  case "ad-hoc":    break;
}
```
...
switch (mode.kind) {
  case "derived":  /* iterate sources, generateDerivedRecord */ break;
  case "primary":  /* Unreachable: the explicit primary check above returns first. */ break;
  case "ad-hoc":   /* fall back to generateAndStorePrimary(schema, null) */ break;
}
```
...
| **(ii) Existing tests that shift (paths)** | None I could find. | None I could find (but with mild risk in `tests/unit/core/derived-identity.test.ts`, `tests/unit/core/derived-no-source-store.test.ts`, `tests/integration/media-library/world.ts` — every world that registers derived schemas would need re-thinking if it also has a primary reg for the *same* reference, which none do today). | None. | None — no current test dual-registers. |
