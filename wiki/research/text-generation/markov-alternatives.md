...
> implementation cost, anchored to _measured_ repo data. Recommends a direction and a
...
was not built at measurement time), so they represent the _shipped, uncompressed,
parsed-by-V8_ form. Numbers are bytes unless noted.
...
six-decimal text (`v.toFixed(6)`), _regardless of how many successors actually occurred_.
...
- **Non-constant PRNG consumption.** Each emitted character = one `prng.random()` (the
  ...
  _data-dependent, variable_ number of PRNG draws (roughly `wordLen + stopChecks`, ×
...
Store _real_ first/last names as plain lists; compose fields (e.g. `firstName ×
...
- **Uniqueness:** ✅ **The decisive advantage.** A Feistel/FPE over `[0, N)` is a
  _bijection_ → guaranteed non-repeating enumeration over the whole product space, which
  Markov fundamentally cannot offer. Ideal for "give me 10,000 distinct users."
- **Determinism-fit:** ✅ Excellent and _constant-consumption_: the permutation is a pure
...
Store the real lists (from §2.1) compactly: a **DAWG/DAFSA** (merge shared prefixes _and_
...
- **Size:** ✅ Large win. _Measured_ gzip on the current corpus already hits 6.9×
  (2.34 MB → 356 KB) on the _wasteful_ CDF text; on plain newline wordlists the gain is
  ...
  **Estimate (reasoned):** ~5k real names ≈ 35 KB raw → front-coding on sorted lists
  ...
  current 2.34 MB, while shipping _real_ data. (Estimates; exact numbers require building
...
character n-grams. This is the _smarter_ version of the abandoned 3-state phoneme
...
- **Determinism-fit:** ✅ Cleaner than char-Markov: bounded, near-constant draws (one per
  ...
  edits.
- **Implementation cost:** 🟡 Needs a syllabification pass at _training_ time to build the
  syllable inventory + transition table (non-trivial tooling, language-specific). Runtime
  sampler is simpler than today's. **Best fit for pseudo-_words_** (nouns/adjectives)
...
- **Quality:** 🟡 Unchanged-to-slightly-better. Pruning the all-prior tail actually
  _reduces_ the chance of drawing a nonsense character; quantization to int16 is
...
- **Size:** ✅ Large, low-risk win. Replacing 6-decimal text (~9 bytes/value) with int16
  ...
  with the _measured_ 6.9× gzip ratio, which is essentially the entropy floor this
...
- **Determinism-fit:** ✅ Same sampler shape, same `fork(key)` usage — **but pruning/
  ...
  versioned retrain, but it _is_ an observable behavior change (D4 keys are fine; the
  _values_ shift).
...
- **Size:** ❌ Even a "tiny" char-RNN is hundreds of KB–MB of weights, _plus_ a runtime
...
- **Verdict:** **Dismiss.** Documented here so it's settled: a zero-dependency,
  deterministic, byte-tiny table/list approach beats a neural model on _every_ axis this
...
- **Uniqueness:** 🟡 Words repeat naturally (real text repeats words); for _unique_
...
- **Verdict:** **Real lists + weighted PCFG win for words.** Markov-invented words are the
  worst of both: gibberish _and_ model bloat.
...
_reference identity_. `Prng.fork(key)` derives an independent child PRNG from
...
1. **Seed the permutation from the field PRNG.** The field already has a forked PRNG
   ...
   key _is_ the permutation's identity — derived purely from the determinism tuple, so it
...
2. **Pick the i-th value.** For the i-th record using this schema slot, compute
   ...
   `prng.ts`). A few (3–4) rounds give a thorough shuffle; this is _not_ security-grade
...
consumption is _constant and structural_ (one permutation key draw + arithmetic), versus
Markov's _data-dependent, variable_ draw count (§1.5) that silently shifts whenever the
...
advances per call on a schema reference) as the Feistel input, so order across _distinct_
...
any PRNG draw; the PRNG only ever drives an _index_ or a `prng.pick`. They slot in without
touching D4/D10 — the permutation/index machinery of §3.1 sits on top unchanged.

### 3.3 Quantize/prune (option 4)

Same `fork(key)` shape as today (✅ for D4/D10 _keys_), but re-encoding the CDF **changes
the values drawn** for a given seed and **changes how many draws a word consumes**. That's
a deterministic-_output_ change — fine as a versioned retrain (seeds change, like the
...
1. **Names → real lists + combinatorial product + Feistel/FPE (options 1 + 2).** This is
   the strongest fit on _every_ axis the library values: best quality (real names), big
   ...
   Markov path cannot do. It is also the _cleanest, most robust_ determinism fit (constant
...
3. **Quantize + prune (option 4) as an immediate, low-risk stopgap** to cut the current
   2.34 MB by an estimated 4–8× _before_ the larger refactor lands — useful if bundle size
...
A **research/measurement spike** (a `chore`/`research` follow-up, _not_ yet a full
...
- **Touches:** `packages/locale-names/` (the five origin groups) and
  ...
  `src/prng.ts` are _read_ for the determinism design (the Feistel helper would live
  beside `prng.ts`).
- **Deliverables:** (a) take the _actual_ training corpora behind today's models (or the
  ...
  Feistel/FPE bijection over a real `N` (verified by an enumeration test, _via the
  test-writer flow_, not an ad-hoc node script) confirming non-repeating + deterministic;
...
- **Implies shipping new data?** **Yes — likely.** Moving from trained models to real
  curated wordlists changes _what data ships_ and its licensing/provenance. That is a
...
- **B2 (Markov entropy for _unkeyed_ synthetic strings):** complementary. B2 targets
  fields _not_ covered by key heuristics — a different surface. If words move to real
...
- **B42 (nl initial-letter A/B/C/D skew, issue #24):** a recommended direction here can
  **subsume B42.** The skew is a property of the start-state distribution baked into the
  ...
  `b`≈4.3 %, … — the empty-state CDF _is_ the start-letter distribution, and it is what
  ...
  the initial-letter distribution is simply the _real_ distribution of the list, by
...
1. **Corpus / data shipping.** Switching names to real wordlists changes _what data
   ships_. Which corpora? (The existing models were trained on SSA names, US Census
   surnames, Meertens/CBS for nl, etc. — are those source lists available to ship
   _directly_, or only as trained models?)
2. **Licensing.** Real wordlists carry licenses (WordNet, OpenTaal GPL/BSD, census public
   domain, Meertens open data). Shipping the _lists_ (not derived models) may have
...
4. **Unique-enumeration semantics by default.** Should `world.populate(Person, 10_000)`
   guarantee 10k _distinct_ names by default (Feistel walk), or keep
