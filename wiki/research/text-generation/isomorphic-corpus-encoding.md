...
> recommendation for any *shipped* path (it assumed Node's `zlib`).
...
*before* the bundler's compressor runs; **after** it runs, they either tie plain
...
The one scheme worth a *maybe* is the **single packed delimited string**
...
tree-shaking only *within a file* (we already split corpora into separate modules),
...
1. **The array wrapper is ~6 bytes/entry of pure boilerplate** — `  "word",\n` is
   ...
   `last-names.ts`, that's ~60 KB of the 124 KB file that is *not* corpus content.
   This is exactly the redundancy a compressor eats for breakfast.
2. **Sort order differs per corpus.** `nouns.ts`/`adjectives.ts` are
   alphabetically sorted (`aahed, aahing, aahs, aalii…`), so front-coding *could*
   ...
   is cosmetic; but the point stands that the largest corpus is the *worst* case
...
english surname corpus *content* (newline-joined, no JS wrapper):
...
That **3.5×–4.6×** is the ratio a bundler achieves on the string *content*
...
For the *current* 10K-entry english surname module (frequency-sorted, ~124 KB raw
source): expect **~38–45 KB brotli / ~52 KB gzip over the wire** as a plain array.
That is the number every other scheme has to beat *after the bundler runs*.
...
- **The bundler is already the brotli stage.** B46's `fc+brotli` win (150,840 vs
  195,155 = ~23 % under plain brotli) was real — but it stacked a *custom*
  front-coding pass *underneath* Node's brotli. Under D13 we can't run brotli at
...
- **DAFSA (#4) breaks the access model.** `prng.pick` (see
  ...
  into a *materialized* `readonly string[]`, one PRNG draw per call (B46 §4). A
  ...
  pay DAFSA's decode cost *and* end up with the same array plain arrays give you
...
- **Packed string (#2) is the only marginal real win.** It removes the
  `"…",\n` wrapper tokens *from the source the bundler sees*, so even after
...
- **None.** The recommendation is to keep the in-flight plain-`string[]` baseline,
  ...
  decision is required to *stay the course*; B50 can close as a null-result research
...
- **N-1 — Adopt the packed `"\n"`-string encoding (#2)?** ~5–12 % over-the-wire
  ...
  `chore`/`lite` implementation item *only if* the maintainer wants the marginal
...
- **N-2 — Largest-corpus sizing.** Orthogonal to encoding: the 10K-entry
  ...
  more than surname variety, trimming to top ~2–5K surnames beats *any* encoding
...
- **N-3 — Revisit only if corpora grow ~10×.** If a future locale ships a
  multi-hundred-KB corpus, re-open the packed-string question (and *only* the
