...
| 9   | `email` (generator)               | [internet.ts:101-126](../../../src/generators/data/internet.ts#L101) | 26      | ~5              | Six-stage sibling fallback (nickname → first/last → company → default). Each stage has its own `domain()` reference and ad-hoc transforms. Not deeply complex — just dense.                                                                                                                                  | Pull the sibling-priority list into a data-driven helper `siblingLocalPart(ctx, fallback)` that returns the email local part and lets `email` just join. Same shape used in `username`, `displayName` — generalise once.                                                                                               |
...
| 2   | [src/types.ts](../../../src/types.ts)                                               | 430      | **25**             | 4                     | Fan-in is justified — every module imports types here. But it bundles unrelated interfaces (`Registry`, `GeneratorContext`, `MatcherCtx`, `GenerateOptions`, `WorldOptions`, `SchemaOpts`, `ExplainResult`). Three of the seven major interfaces are >50 lines of JSDoc + types.    | Split into `types/context.ts`, `types/options.ts`, `types/world.ts`, `types/explain.ts`. Keep a barrel `types/index.ts`. Mostly aesthetic — does not change fan-in.                                                                                                                                                                                                                                                                |
...
**Dependency topology** is clean: one big consumer (`world.ts` imports 12), one big producer (`types.ts` is imported by 25), no circular imports, no surprising cross-cuts. The locale workspaces are coupled to `locale-core` and nothing else.
...
The 0-through-6 pipeline is the engine's contract. It is documented in three places: `docs/concepts.md` (rendered docs), `wiki/codebase-map.md` (internal map), and the module-level JSDoc at the top of [src/world.ts:14-26](../../../src/world.ts#L14). Plus `CLAUDE.md`. The JSDoc, however, lists **7 steps**, not 6 (it numbers the per-schema key map as step 2 and bumps the rest down). `docs/concepts.md` lists 6. **The two documents disagree on numbering**, and the code actually implements 7 ladder rungs (override eager → matcher → schemaKeyMap → optional-unwrap → custom-gen → key-based → schema-based).
...
- Pipeline numbering: 6 steps in `docs/concepts.md`, 7 in `world.ts` JSDoc. Code implements 7 rungs (the `withGenerators` custom-key step isn't called out anywhere in the rendered ladder).
