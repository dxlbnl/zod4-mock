...
## D27 — API reference is generated by TypeDoc; the ts-morph generator + `docs:check` parity guard are retired (amends D5, supersedes D24)

- **Date**: 2026-06-07.
- **By**: implementer (B125); flagged for the manager to promote the amended D5 rule and retire
  the D24 rule at Done.
- **Context**: The maintainer-approved docs-UX plan
  (`wiki/research/reports/docs-ux-rework.md`) replaces the bespoke `ts-morph` API-docs pipeline
  (B102 extractor + `manifest.generated.ts` + the `docs/api-reference.md` parity guard, extended
  by B115 grouping and B123 TOC nesting) with **TypeDoc**, which natively produces the
  member-level reference the project needs — functions with expanded options, option/config types
  with every field, interfaces with every method, all cross-linked. A worktree spike proved
  TypeDoc runs clean on the real library (TS 6, ESM, Zod v4 internals, workspace dep) and resolves
  the full public surface only when pointed at the **real `src/index.ts`** (the `node_modules`
  symlink resolves a handful of nodes), with `sources.fileName` joining against `src/` once the
  TypeDoc tsconfig maps the package name → `src` (D5/D24 mandated the opposite tool).
- **Decision**:
  - The **API reference is generated by TypeDoc**, configured (`site/typedoc.json` +
    `site/typedoc.tsconfig.json`) to read the real `src/index.ts` entry, emitting a **JSON** model
    (`site/src/lib/docs/api/typedoc.json`) and an **HTML** link target (`site/static/api-html/`,
    B126's future type-link target). A build-time transform
    (`site/scripts/build-api-model.ts`) distills the JSON into a small, runtime-agnostic render
    model (`api-model.generated.ts`, D13-compliant plain data, committed) that the `/docs/api`
    `+page.svelte` renders member-level in-site (`@dxlbnl/ui` look + Pagefind search preserved).
  - **TSDoc on the exported `src/` symbol stays the prose source of truth** — the D5 same-step
    obligation survives: a public API change updates the symbol's TSDoc in `src/` in the same step,
    and that TSDoc flows into TypeDoc.
  - A **build-time dangling-link guard** (`site/scripts/api-link-guard.ts`, wired into the site
    `build`) fails the build when any `/docs/api` cross-reference resolves to a missing anchor
    (B119-class defect), replacing the `docs:check` parity guard.
  - `typedoc` is a **build-time** `site/` devDependency (D13-exempt, like `pagefind`/`ts-morph`
    were); it never enters the shipped library or the site client bundle.
  - The bespoke pipeline is **deleted**: `scripts/docs/extract.ts`, `scripts/docs/curation.ts`,
    `scripts/docs-generate.ts`, `site/src/lib/docs/api/manifest.generated.ts`, the
    `SignatureBlock`/`ParameterTable` widgets + stories, the old `/docs/api` renderer, the root
    `docs:generate`/`docs:check` scripts (and `docs:check` from `pnpm validate`), and the
    `ts-morph` root devDependency. B115 grouping + B123 TOC nesting are reverted.
  - `docs/api-reference.md` is **kept as a committed static markdown reference** (it is
    cross-linked from README and several `docs/` pages with stable anchors); it is no longer
    auto-regenerated by the deleted pipeline. The canonical living reference is the in-site
    `/docs/api`.
- **Consequences**: D24's mandate (use `ts-morph`, not TypeDoc) is now the opposite of reality and
  is superseded/retired. D5's references to `pnpm docs:generate` / `pnpm docs:check` /
  `manifest.generated.ts` no longer hold; D5's surviving content is the TSDoc same-step
  obligation. The reviewer confirms this ADR exists; the manager promotes the amended D5 + retires
  D24 in `architecture.md`'s Rules section at Done.
- **Rule added/changed**: the manager promotes at Done, e.g. amended D5: "When a public API
  changes, the exported symbol's **TSDoc** in `src/` **MUST** be updated in the same step; the
  in-site `/docs/api` reference is generated from that TSDoc by **TypeDoc** (build-time
  `site/` devDependency, D13-exempt) via `site/typedoc.json` → JSON model →
  `api-model.generated.ts`, and a build-time dangling-link guard (`site/scripts/api-link-guard.ts`)
  fails the build on a dead `/docs/api` anchor. (→ D5)" and **retire D24** (the `ts-morph`
  mandate).
- **Supersedes**: amends D5; supersedes/retires D24.

## D28 — The `zod4-mock` barrel exports only the user-facing public surface; `extend` lives with the locale packages

- **Date**: 2026-06-08.
- **By**: maintainer-directed (B129), promoted by the manager at Done.
- **Context**: The rebuilt `/docs/api` (B125) faithfully renders whatever `src/index.ts` exports,
  which surfaced that the barrel was re-exporting engine internals and a misplaced symbol. Tracing
  each: `generateFromSchema`/`generateFromKey` (the pipeline "rungs") and `fieldSeed` have **no user
  call path** — they require a hand-constructed `GeneratorContext`/seed the engine builds internally;
  `data` is a literal duplicate of `generators` (both `= dataNs`); `extend` is a locale concern
  (defined in `@zod4-mock/locale-core`, merely re-exported by the barrel) and a user only reaches for
  it with a base locale (`extend(en, …)`) already imported from `@zod4-mock/locale-en`.
- **Decision**: hard-remove `generateFromSchema`, `generateFromKey`, `fieldSeed`, and `data` from the
  `zod4-mock` barrel (their definitions stay for internal use + deep-path tests); relocate `extend` to
  re-export from `@zod4-mock/locale-en` and `@zod4-mock/locale-nl` (canonical definition stays in
  `@zod4-mock/locale-core`) and drop it from the main barrel. Kept public: `generate`, `createWorld`,
  `createPrng`, `generators`, `DEFAULT_KEY_MAP`, `DEFAULT_KEY_PATTERNS`, and the type surface.
- **Consequences**: a **major** SemVer bump (breaking for `import { … } from "zod4-mock"` of a removed
  symbol; `import { extend } from "zod4-mock"` → `from "@zod4-mock/locale-en"`). `tests/unit/public-api-surface.test.ts`
  pins the barrel shape. The `/docs/api` reference is automatically curated since it renders the model.
- **Rule added**: promoted to `architecture.md` Rules — the barrel exports only the user-facing public
  surface; the engine helpers are not re-exported; `extend` ships from the locale packages. (→ D28)

## D29 — Engine-threaded non-public option fields are `@internal` + excluded from the reference

- **Date**: 2026-06-08.
- **By**: maintainer-directed (B125 render/curation), promoted by the manager at Done.
- **Context**: `GenerateOptions` carries `source`/`fieldPath`/`prng` only because the engine reuses
  the type when recursing — a user never sets them. Deleting them from the type would break internal
  code; leaving them documented pollutes the reference (alphabetical order even floated the internal
  `fieldPath` to the second row). Separately, `GenerateOptions` and `WorldOptions` shared 5 identical
  fields, rendering as two near-duplicate blobs.
- **Decision**: tag engine-threaded, no-user-path option fields `@internal` and set TypeDoc
  `excludeInternal: true` so they stay in the type (structural-identity plumbing, non-breaking) but
  leave the `/docs/api` reference. Extract the 5 shared fields into an exported base interface
  `GenerationDefaults`; `WorldOptions extends GenerationDefaults` and `GenerateOptions<T> extends
GenerationDefaults` add only their own fields. The reference shows each type's own fields plus a
  compact "Inherited from GenerationDefaults" link row, documenting the shared fields once.
- **Consequences**: `GenerationDefaults` is a new exported public type (additive, minor); the
  `@internal`+`excludeInternal` exclusion is the convention for engine-threaded non-public fields.
