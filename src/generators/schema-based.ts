...
function checks(schema: ZodTypeAny): ZodCheck[] {
  const raw = def(schema).checks as
    | Array<{ _zod: { def: ZodCheck } }>
    | undefined;
...
export function generateFromSchema(
  ...
  switch (d.type) {
    case "string": {
      ...
      if (format === "url")
        return `https://example.com/${generateString(prng, 3, 10).replace(/ /g, "-")}`;
    ...
    case "pipe": {
      ...
      return pipeIn
        ? generateFromSchema(pipeIn, ctx)
        : generateString(prng, 3, 20);
    }
