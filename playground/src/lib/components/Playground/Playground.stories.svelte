...
<Story
	...
	play={async ({ canvasElement }) => {
		...
		const identityArea = canvas.getByText("Identity Source:").parentElement as HTMLElement;
		await expect(within(identityArea).getByRole("button", { name: /User/i })).toBeInTheDocument();
		...
		const linkBtn = within(userIdLine).getByRole("button", {
			name: /Map to subject field/i,
		});
