...
function makeDefaultState(): PlaygroundState {
	...
	const orderSubject: SubjectDef = {
		...
		fields: [
			...
			makeField({ key: 'status', type: 'enum', enumValues: ['pending', 'shipped', 'delivered', 'cancelled'] }),
			makeField({ key: 'totalCents', type: 'number', modifiers: [{ name: '.int()' }, { name: '.min', value: 0 }] }),
			makeField({ key: 'createdAt', type: 'date' }),
		],
	};
	...
	const productSubject: SubjectDef = {
		...
		fields: [
			makeField({ key: 'id', type: 'uuid' }),
			makeField({ key: 'name', type: 'string', modifiers: [{ name: '.min', value: 2 }, { name: '.max', value: 100 }] }),
			makeField({ key: 'sku', type: 'string' }),
			makeField({ key: 'priceCents', type: 'number', modifiers: [{ name: '.int()' }, { name: '.min', value: 1 }] }),
...
export function createPlaygroundState(initial?: PlaygroundState) {
	...
	return {
		...
		get builderTitle() { return builderTitle; },
		get activeBinding() { return activeBinding; },
