...
The goal is to test edge cases (e.g., *"What if this field is null?"*) without polluting the primary schema or world setup.
...
The UX answers the most common question: *"Why is this value specifically 'John'?"*
...
-   **Seed Lineage Popover**: A clean, technical popover appears showing the derivation:
    ...
    -   `Final Hash: 0x8a2f...`
-   **Re-Roll Simulation**: A button to "Test different seed" for *just this field* to see what other values the PRNG would produce from the current schema.
...
| :--- | :--- | :--- |
| **Sandbox Diffing** | JSON Override | *"I tested the 'error' state without breaking my clean schema."* |
| **PRNG Tracing** | Click "Math" icon | *"Now I understand why this value is stable and how seeds work."* |
| **Heuristic Search** | Type field name | *"I see why 'email' is auto-generated—there's a rule for it."* |
