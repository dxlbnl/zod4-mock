...
const FileSubject = defineSubjectType('file', z.object({
...
describe('overrides — top-level fields', () => {
  it('overrides a scalar field', () => {
    const file = makeWorld().generate(FileSchema, { overrides: { status: 'failed' } })
    expect(file.status).toBe('failed')
  })

  it('does not affect other fields when overriding one', () => {
    const baseline = makeWorld().generate(FileSchema)
    const overridden = makeWorld().generate(FileSchema, { overrides: { status: 'failed' } })
...
describe('transform', () => {
  ...
  it('receives the full generated object', () => {
    ...
    makeWorld().generate(FileSchema, {
      transform: (data) => { calls.push(data); return data },
    })
...
describe('overrides + transform combined', () => {
  it('overrides are applied before transform', () => {
    ...
    makeWorld().generate(FileSchema, {
      ...
      transform:  (data) => { seenStatus = data.status; return data },
    })
  ...
  it('transform receives overridden values', () => {
    ...
    makeWorld().generate(FileSchema, {
      ...
      transform:  (data) => { seen.push(data.metadata.uploadedBy); return data },
    })
