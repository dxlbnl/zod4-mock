...
describe("generators namespace", () => {
  ...
  it("contains all primitive generator functions", () => {
    ...
    for (const name of expected) {
      expect(
        typeof (generators as Record<string, unknown>)[name],
        `generators.${name}`,
      ).toBe("function");
    }
  });
  ...
  it("primitive generators produce different values for different seeds", () => {
    expect(generators.firstName(createPrng(1))).not.toBe(
      generators.firstName(createPrng(2)),
    );
    expect(generators.email(createPrng(1))).not.toBe(
      generators.email(createPrng(2)),
    );
  });
});
...
describe("case-insensitive key matching", () => {
  ...
  const MixedSubject = defineSubjectType(
    "mixed",
...
describe("schema-gated custom generators", () => {
  ...
  const GatedSubject = defineSubjectType(
    "gated",
    z.object({ name: z.string() }),
  );
...
describe("KeyGenerator type", () => {
  it("can be used as a type annotation", () => {
    ...
    const gen: KeyGenerator<string> = (_schema, ctx) =>
      generators.firstName(ctx.prng);
