...
**Whenever a public API changes — new method, removed method, changed signature, or changed behaviour — update the exported symbol's TSDoc in `src/` in the same step.** The TSDoc on each `src/` public export (and the files `src/index.ts` re-exports) is the prose source of truth. The in-site API reference at `/docs/api` is generated from that TSDoc by **TypeDoc** (a build-time `site/` devDependency): the site `build` runs `typedoc` (config: `site/typedoc.json` + `site/typedoc.tsconfig.json`, entry = the real `src/index.ts`) → a JSON model → `site/scripts/build-api-model.ts` distills it into `site/src/lib/docs/api/api-model.generated.ts`, which the `/docs/api` `+page.svelte` renders member-level. A build-time dangling-link guard (`site/scripts/api-link-guard.ts`, run in the site `build`) fails the build on a dead `/docs/api` cross-reference anchor. Signatures are derived from the real types, never hand-typed; do NOT hand-edit `api-model.generated.ts`. `docs/api-reference.md` is a committed static markdown reference (no longer auto-regenerated); the canonical living reference is `/docs/api`. Do not defer documentation to a follow-up; it will be forgotten. (See D5/D27 in `wiki/decisions.md`; D24 is superseded.)
