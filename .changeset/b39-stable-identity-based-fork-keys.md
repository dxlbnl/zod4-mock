...
For most consumers this is invisible — the in-repo test suite required zero functional re-pins (three tests were restructured to hoist schema construction, preserving their intent under the new contract). But the PRNG sequence is part of the published deterministic contract, so any downstream snapshot test against the changed paths will produce different (still-deterministic) values. Hence the `major` bump.
